 ...
 */
public class Sentinel extends ShootingEnemy
 {
	/**
	 ...
	 * Shot time
	 */
	public static final float TIME = 1500;
	
	/**
	 * Shot sound
	 */
	private static Sound shotSound = Gdx.audio.newSound(Gdx.files
			.internal((PropertiesManager.getParameter("gunShot"))));
	 ...
	 */
	public Sentinel(Framework framework, Vector2 position, Shot shot, ShotDecorator shotDec)
	{
		super(framework, position, ENERGY_VALUE, SCORE_VALUE, DAMAGE_VALUE,
				SPEED_VALUE, "sentinel",
	 ...
	 */
	public Sentinel(Framework framework, Vector2 position, Shot shot, ShotDecorator shotDec, int maxShots)
	{
	 ...
	 */
	public Sentinel(Framework framework, Vector2 position, Shot shot, ShotDecorator shotDec, boolean unlimited)
	{
		this(framework, position, shot, shotDec);
	...
	/**
	 * Handle the collision with an other physics entity, damaging it if it is the ship (1.5 points)
	 * @param e the physics entity with which collides
	 */
	@Override
	public void handleContact(DynamicPhysicsEntity e)
	{
		if (e instanceof Ship) e.damage((float)1.5);
